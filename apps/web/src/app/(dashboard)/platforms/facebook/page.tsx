...
export default function FacebookPage() {
  ...
  const loadData = async () => {
    try {
      const [accountsData, analyticsData] = await Promise.all([
        api.getPlatforms({ platform: 'facebook' }),
        api.getAnalyticsPlatforms('facebook').catch(() => null),
      ]);

      const fbAccounts = Array.isArray(accountsData)
        ? accountsData
      ...
      if (fbAccounts.length > 0) {
        ...
        // Load recent posts for first account
        loadPosts(fbAccounts[0].id);
      }
    } catch (err) {
  ...
  const handleConnectAccount = () => {
    // Redirect to OAuth flow or show manual token input
    alert('OAuth integration: Will redirect to Facebook OAuth flow');
    // In production: window.location.href = '/api/platforms/facebook/connect';
  };
  ...
  const handlePost = async () => {
    if (!selectedAccount || !content) {
      alert('Please select an account and enter content');
      return;
    }
  ...
  const handleCreateCampaign = () => {
    router.push('/campaigns?platform=facebook');
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Manage your Facebook accounts, posts, and campaigns
          </p>
        </div>
        <button
        ...
        <div className="p-6">
          {accounts.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-500 mb-4">No Facebook accounts connected</p>
              <button
                onClick={handleConnectAccount}
                className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg"
              >
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center text-white text-xl font-bold">
                      {account.accountName?.[0] || 'F'}
                    </div>
                    <div>
                      <div className="font-semibold text-gray-900 dark:text-white">
                        {account.accountName || account.username || 'Facebook Account'}
                      </div>
      ...
      {accounts.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div className="flex gap-4">
            ...
            <button
              onClick={() => router.push('/analytics?platform=facebook')}
