  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const [overviewData, notificationsData] = await Promise.all([
        ...
        api.getNotifications({ limit: '3' }).catch(() => ({ notifications: [] })),
      ]);

      setStats({
        workflows: overviewData.totalCampaigns || 0,
        platforms: overviewData.platformCount || 0,
        postsPublished: overviewData.totalPosts || 0,
        activeWorkflows: overviewData.activeWorkflows || 0,
      });

      setRecentActivity(notificationsData.notifications || []);
    } catch (error) {
